...
set personal_taste_line = nullif(
  regexp_replace(personal_taste_line, '^[[:space:]]+|[[:space:]]+$', '', 'g'),
  ''
)
...
alter table public.profiles
  ...
  check (
    personal_taste_line is null
    or char_length(regexp_replace(personal_taste_line, '^[[:space:]]+|[[:space:]]+$', '', 'g')) between 1 and 160
  );
...
begin
  if new_personal_taste_line is null then
    ...
    normalized_taste_line := regexp_replace(
      new_personal_taste_line,
      '^[[:space:]]+|[[:space:]]+$',
      '',
      'g'
    );
