          ...
          <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 shadow-sm">
            <div className="grid gap-0 md:grid-cols-[0.95fr_1.05fr]">
              ...
              <div className="p-8 md:p-10 space-y-6">
                <div>
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-primary-amber/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-widest text-primary-amber">
                    <Sparkles size={11} />
                    {state.card.badges[0] ?? coffeeDexBrand.name}
                  </span>
                  <h1 className="mt-4 font-serif text-4xl font-extrabold leading-tight text-foreground">{state.card.title}</h1>
                  <p className="mt-2 text-sm font-bold text-muted-foreground/80">{state.card.subtitle}</p>
                </div>
                ...
                <p className="rounded-3xl border border-white/10 bg-white/5 p-5 font-serif text-sm italic leading-relaxed text-foreground/90">
                  “{state.card.ai_description || "사용자가 확인한 CoffeeDex 테이스팅 기록입니다."}”
                </p>

                <div className="flex flex-wrap gap-2">
                  {state.card.tags.slice(0, 5).map((tag) => (
                    <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-bold text-muted-foreground">#{tag}</span>
                  ))}
                </div>
              </div>
            </div>
