...
function NavigationItems({ activeTab, onTabChange }: DashboardNavigationProps) {
  return (
    <>
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isActive
                ? "bg-background-dark text-[#FFF8EC] shadow-[0_10px_24px_rgba(73,48,36,0.18)]"
      ...
      <Link
        href="/settings"
        className="flex min-h-12 flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[11px] font-bold text-muted-foreground transition-[color,background-color,transform] hover:bg-background-dark/6 hover:text-background-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-amber active:scale-95 md:min-h-10 md:flex-none md:flex-row md:gap-2 md:px-4 md:text-xs"
      >
...
export function DashboardDesktopNavigation(props: DashboardNavigationProps) {
  return (
    <nav aria-label="대시보드 주요 메뉴" className="hidden items-center gap-1 rounded-[1rem] bg-white/35 p-1 md:inline-flex">
...
export function DashboardMobileNavigation(props: DashboardNavigationProps) {
  return (
    ...
    >
      <div className="mx-auto flex max-w-md items-center gap-1 rounded-[1.25rem] border border-background-dark/10 bg-white/72 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
        <NavigationItems {...props} />
      </div>
    </nav>
  );
