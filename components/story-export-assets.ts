...
function metricSvg(label: string, value: number, y: number, skin: StorySkin): string {
  ...
  return `<text x="180" y="${y}" font-size="30" font-family="Inter, Arial" fill="${skin.svg.muted}">${label}</text><text x="900" y="${y}" text-anchor="end" font-size="30" font-family="Inter, Arial" fill="${skin.svg.text}">${value} / 5</text><rect x="180" y="${y + 28}" width="720" height="12" rx="6" fill="${skin.svg.border}"/><rect x="180" y="${y + 28}" width="${width}" height="12" rx="6" fill="${skin.svg.accent}"/>`;
}

function tagSvg(tag: string, index: number, skin: StorySkin): string {
  const x = 180 + index * 205;
  return `<rect x="${x}" y="1110" width="180" height="52" rx="26" fill="${skin.svg.badge}" stroke="${skin.svg.border}"/><text x="${x + 90}" y="1145" text-anchor="middle" font-size="26" font-weight="700" font-family="Inter, Arial" fill="${skin.svg.text}">#${escapeSvg(truncateText(tag, 10))}</text>`;
}
...
export function storySvg(card: TastingCardData, skin: StorySkin, dateText: string, originText: string): string {
  ...
  const quoteSvg = quote.map((line, index) => `<text x="180" y="${1250 + index * 44}" font-size="32" font-style="italic" font-family="Georgia, serif" fill="${skin.svg.text}" opacity="0.86">${index === 0 ? "“" : ""}${escapeSvg(line)}${index === quote.length - 1 ? "”" : ""}</text>`).join("");
  const tags = card.tags.slice(0, 3).map((tag, index) => tagSvg(tag, index, skin)).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920" viewBox="0 0 1080 1920"><rect width="1080" height="1920" fill="${skin.svg.background}"/><circle cx="80" cy="50" r="210" fill="${skin.svg.accent}" opacity="0.12"/><circle cx="980" cy="1810" r="260" fill="${skin.svg.accent}" opacity="0.10"/><text x="96" y="140" font-size="26" font-weight="800" letter-spacing="8" font-family="Inter, Arial" fill="${skin.svg.accent}">HYANGMI ARCHIVE</text><text x="984" y="140" text-anchor="end" font-size="24" font-family="Inter, Arial" fill="${skin.svg.muted}">${escapeSvg(dateText)}</text><rect x="96" y="390" width="888" height="900" rx="58" fill="${skin.svg.panel}" stroke="${skin.svg.border}" stroke-width="3"/><rect x="160" y="470" width="230" height="54" rx="14" fill="${skin.svg.badge}"/><text x="275" y="506" text-anchor="middle" font-size="24" font-weight="800" font-family="Inter, Arial" fill="${skin.svg.accent}">${escapeSvg(card.badges[0] || "Single Origin")}</text><rect x="762" y="470" width="134" height="134" rx="32" fill="${skin.svg.badge}" stroke="${skin.svg.border}" stroke-width="3"/><text x="829" y="552" text-anchor="middle" font-size="60" font-family="Georgia, serif" fill="${skin.svg.accent}">${escapeSvg(card.title.slice(0, 1).toUpperCase())}</text><text x="160" y="615" font-size="56" font-weight="800" font-family="Georgia, serif" fill="${skin.svg.text}">${escapeSvg(truncateText(card.title, 22))}</text><text x="160" y="674" font-size="32" font-family="Inter, Arial" fill="${skin.svg.muted}">${escapeSvg(truncateText(card.subtitle, 34))}</text>${metricSvg("산미 (Acidity)", card.metric1, 790, skin)}${metricSvg("단맛 (Sweetness)", card.metric2, 900, skin)}${metricSvg("바디감 (Body)", card.metric3, 1010, skin)}${tags}<line x1="160" x2="920" y1="1208" y2="1208" stroke="${skin.svg.border}" stroke-width="3"/>${quoteSvg}<text x="540" y="1580" text-anchor="middle" font-size="28" font-family="Inter, Arial" fill="${skin.svg.muted}">Origin: ${escapeSvg(originText)}</text><line x1="500" x2="580" y1="1625" y2="1625" stroke="${skin.svg.border}" stroke-width="3"/><text x="540" y="1688" text-anchor="middle" font-size="22" font-weight="800" letter-spacing="6" font-family="Inter, Arial" fill="${skin.svg.muted}">PUBLIC HYANGMI CARD</text><text x="540" y="1730" text-anchor="middle" font-size="22" font-weight="800" letter-spacing="6" font-family="Inter, Arial" fill="${skin.svg.muted}">SHARED VIA HYANGMI</text></svg>`;
}
