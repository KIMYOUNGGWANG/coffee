...
function metricSvg(label: string, value: number, y: number, skin: StorySkin): string {
  ...
  return `<text x="180" y="${y}" font-size="30" font-family="${storySansFont}" fill="${skin.svg.muted}">${label}</text><text x="900" y="${y}" text-anchor="end" font-size="30" font-family="${storySansFont}" fill="${skin.svg.text}">${value} / 5</text><rect x="180" y="${y + 28}" width="720" height="12" rx="6" ry="6" fill="${skin.svg.border}"/><rect x="180" y="${y + 28}" width="${width}" height="12" rx="6" ry="6" fill="${skin.svg.accent}"/>`;
}

function tagSvg(tag: string, index: number, skin: StorySkin): string {
  const x = 180 + index * 205;
  return `<rect x="${x}" y="1110" width="180" height="52" rx="26" ry="26" fill="${skin.svg.badge}" stroke="${skin.svg.border}"/><text x="${x + 90}" y="1145" text-anchor="middle" font-size="26" font-weight="700" font-family="${storySansFont}" fill="${skin.svg.text}">#${escapeSvg(truncateText(tag, 10))}</text>`;
}
