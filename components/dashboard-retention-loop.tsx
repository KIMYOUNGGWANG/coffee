...
export function DashboardRetentionLoop({ cards, onQuickAdd, onSelectCard }: DashboardRetentionLoopProps) {
  ...
  return (
    <section className="rounded-[1.75rem] border border-primary-amber/18 bg-[#120d09] p-4 text-[#F7F7F4] shadow-[0_24px_80px_rgba(0,0,0,0.28)] sm:p-5" aria-label="오늘 다시 찾을 기억">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-black text-[#F7F7F4]/44">{cards.length}개 기억 중</span>
          </div>
          <h2 className="mt-3 break-keep font-serif text-2xl font-black leading-tight sm:text-3xl">오늘 다시 찾을 기억</h2>
          <p className="mt-2 break-keep text-sm font-semibold leading-6 text-[#F7F7F4]/62">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-white/12 bg-white/[0.04] px-4 text-sm font-black text-[#F7F7F4] transition hover:-translate-y-0.5 hover:border-primary-amber/35 hover:text-primary-amber"
          >
            <Sparkles size={16} />
            기억 열기
          </button>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        <button
          ...
          className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-left transition hover:border-primary-amber/30"
        >
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-primary-amber/80">다시 살 이유</p>
          <p className="mt-2 break-keep text-base font-black leading-6 text-[#F7F7F4]">
            {rebuyReason ?? "아직 다시 살 이유를 남긴 원두가 없어요."}
          </p>
          <p className="mt-1 truncate text-xs font-semibold text-[#F7F7F4]/45">{(rebuyCard ?? anchorCard).title}</p>
        </button>
        ...
        >
          ...
          <p className="mt-1 truncate text-xs font-semibold text-[#F7F7F4]/45">{(brewCard ?? anchorCard).title}</p>
        </button>
      </div>
    </section>
  );
