...
| `PATCH` | `/api/v1/profile` | Save or clear the authenticated user's private personal taste sentence. |
...
- private Rebuy Clue Rescue on the dashboard that derives a queue of `again`/`maybe` cards missing purchase place, price, purchase link, or rebuy reason, so users can reopen the card or save at least one missing clue inline through `PATCH /api/v1/cards/:id` before that bean becomes hard to buy again;
...
- private Personal Taste Line that lets the owner rewrite the Taste Rebuy Brief in up to 160 Korean-first characters, persist it on the owner-scoped profile, return to the automatic record-derived sentence at any time, and copy either version without placing the sentence or coffee identity in analytics properties;
